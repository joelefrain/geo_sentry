...
def calculate_note_variables(dfs, sensor_names, serie_x, target_column):
...
def create_note(group_args, data_sensors, target_column, unit_target, series_names, serie_x, sensor_aka):
    ...
    calc_vars = calculate_note_variables(data_sensors["df"], data_sensors["names"], serie_x, target_column)
...
def create_cell_1(data_sensors, series_names, primary_column, reference_column, serie_x, primary_title_y, sensor_type_name):
    plotter = PlotBuilder()
    ...
    series_styles = {
        "terrain_level": {
            ...
            "marker": "s",
            "markersize": 2,
            "label_prefix": series_names["terrain_level"],
        },
        reference_column: {
            "color": "dimgrey",
            "linetype": "-",
            ...
            "marker": "s",
            "markersize": 2,
            "label_prefix": series_names[reference_column],
        },
        primary_column: {
            "color": "blue",
            "linetype": "-",
...
def create_cell_2(data_sensors, start_query, end_query, series_names, secondary_column, serie_x):
    plotter = PlotBuilder()
...
def generate_report(
    ...
    reference_column = column_config["reference_column"]
    ...
    chart_cell1, legend1 = create_cell_1(data_sensors, series_names, primary_column, reference_column, serie_x, primary_title_y, sensor_type_name)
    ...
    upper_cell = create_note(
        group_args, data_sensors, target_column, unit_target, series_names, serie_x, sensor_aka
    )
    lower_cell = create_map(dxf_path, data_sensors)
    logo_cell = load_svg(LOGO_SVG, 0.08)
    chart_title = f"Registro de {sensor_type_name} / {' - '.join(data_sensors['names'])} / {group_args['name']} / {geo_structure}"
    ...
    # Generate PDF
    pdf_filename = f"{output_dir}/{appendix}_{start_item}.pdf"
