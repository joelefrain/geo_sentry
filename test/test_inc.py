...
def process_raw_data(df, config):
    """Process matrix values using self_operations and base_operations independently."""
    # Process self operations if present
    if "self_operations" in config:
        df = process_self_operations(df, config)

    if "base_operations" in config:
        df = process_base_operations(df, config)
...
if __name__ == "__main__":
    toml_path = "modules/calculations/data/inc.toml"
    input_csv = "test/inc_test.csv"
    output_csv = "test/inc_test_processed.csv"
    ...
    # Convert 'time' to datetime for accurate comparison
    df["time"] = pd.to_datetime(df["time"])

    # Process matrix values
    processed_df = process_raw_data(df, config)

    # Save processed data
    processed_df.to_csv(output_csv, index=False, sep=";")
    print(f"Processed data saved to: {output_csv}")

    # Create plots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))

    # Plot for each unique time
    for time in processed_df["time"].unique():
        time_data = processed_df[processed_df["time"] == time]

        # A_deflection plot
        ax1.plot(
            time_data["A_deflection"],
            time_data["FLEVEL"],
            label=pd.to_datetime(time).strftime("%Y-%m-%d %H:%M"),
            marker="o",
            linestyle="-",
            markersize=4,
        )
    ...
    # Configure plots
    ax1.set_xlabel("A_deflection")
    ax1.set_ylabel("FLEVEL")
    ax1.set_title("A_deflection vs FLEVEL")
    ax1.grid(True)
    ax1.legend(bbox_to_anchor=(1.05, 1), loc="upper left")

    ax2.set_xlabel("B_deflection")
    ax2.set_ylabel("FLEVEL")
    ax2.set_title("B_deflection vs FLEVEL")
    ax2.grid(True)
